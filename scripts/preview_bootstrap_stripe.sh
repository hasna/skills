...
usage() {
  cat <<'USAGE'
Usage: STRIPE_API_KEY=sk_test_... scripts/preview_bootstrap_stripe.sh

Creates or reuses sandbox Stripe billing resources for skills.md previews and
updates skillsmd/preview/stripe in AWS Secrets Manager. Secrets are never printed.
USAGE
}
...
if [[ "$STRIPE_API_KEY" != sk_test_* && "$STRIPE_API_KEY" != rk_test_* ]]; then
  echo "::error::STRIPE_API_KEY must be a test Stripe secret or restricted key" >&2
  exit 1
fi

STRIPE_PUBLISHABLE_KEY="${STRIPE_PUBLISHABLE_KEY:-$(awk -F"'" '/test_mode_pub_key/{print $2; exit}' "$HOME/.config/stripe/config.toml" 2>/dev/null || true)}"
if [[ "$STRIPE_PUBLISHABLE_KEY" != pk_test_* ]]; then
  echo "::error::STRIPE_PUBLISHABLE_KEY must be a test Stripe publishable key" >&2
  exit 1
fi
...
ensure_price() {
  ...
  if [[ -n "$recurring_interval" ]]; then
    stripe_json prices create --currency usd --unit-amount "$unit_amount" --product "$product_id" --lookup-key "$lookup_key" --recurring.interval "$recurring_interval" | jq -r '.id'
  else
    stripe_json prices create --currency usd --unit-amount "$unit_amount" --product "$product_id" --lookup-key "$lookup_key" | jq -r '.id'
  fi
}

ensure_webhook() {
  local endpoint webhooks_json
  local endpoint_json
  webhooks_json="$(stripe_json webhook_endpoints list --limit 100)" || return 1
  endpoint="$(jq -r --arg url "$PREVIEW_WEBHOOK_URL" '.data[] | select(.url == $url and .status == "enabled") | .id' <<<"$webhooks_json" | head -n1)"
...
pro_price_id="$(ensure_price skillsmd_preview_pro_monthly "$pro_product_id" 1000 month)" || exit 1
credit_1_price_id="$(ensure_price skillsmd_preview_credit_1 "$credits_product_id" 100)" || exit 1
credit_5_price_id="$(ensure_price skillsmd_preview_credit_5 "$credits_product_id" 500)" || exit 1
credit_20_price_id="$(ensure_price skillsmd_preview_credit_20 "$credits_product_id" 2000)" || exit 1
credit_50_price_id="$(ensure_price skillsmd_preview_credit_50 "$credits_product_id" 5000)" || exit 1
credit_100_price_id="$(ensure_price skillsmd_preview_credit_100 "$credits_product_id" 10000)" || exit 1
...
account_id="${STRIPE_ACCOUNT_ID:-$(awk -F"'" '/account_id/{print $2; exit}' "$HOME/.config/stripe/config.toml" 2>/dev/null || true)}"
if [[ -z "$account_id" ]]; then
  echo "::error::STRIPE_ACCOUNT_ID is required when Stripe CLI config does not contain account_id" >&2
  exit 1
fi
...
echo "Preview Stripe resources are configured in AWS Secrets Manager."
