...
| `HASNA_SKILLS_SERVER_DATABASE_POOL_MAX` plus legacy `HASNA_SKILLS_DATABASE_POOL_MAX` / `SKILLS_DATABASE_POOL_MAX` | Provider-neutral server database pool size | Resolve the target server-scoped name before the two legacy names and the default `4`. |
| `HASNA_SKILLS_SERVER_S3_*` plus legacy `HASNA_SKILLS_S3_*` / `SKILLS_S3_*` | Provider-neutral server authoritative object store | The target server-scoped bucket, prefix, endpoint, path-style, and credential references win; legacy names are compatibility reads only and must not be conflated with client sync. |
| `HASNA_SKILLS_SERVER_AWS_REGION` plus legacy `AWS_REGION` | Provider-neutral server object-store region | The target server-scoped region wins. Do not use client-sync `HASNA_SKILLS_AWS_REGION` or `SKILLS_AWS_REGION` as an implicit server fallback. |
