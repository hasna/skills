...
| `HASNA_SKILLS_DATABASE_URL` / `DATABASE_URL` | Provider-neutral server authoritative database reference | Preserve this namespace separately from client sync. Server and migration resolution is exactly `HASNA_SKILLS_DATABASE_URL` over `DATABASE_URL`. |
| `HASNA_SKILLS_DATABASE_POOL_MAX` / `SKILLS_DATABASE_POOL_MAX` | Provider-neutral server database pool size | Resolve exactly `HASNA_SKILLS_DATABASE_POOL_MAX` over `SKILLS_DATABASE_POOL_MAX` over the default `4`. |
