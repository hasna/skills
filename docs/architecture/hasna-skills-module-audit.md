...
This audit defines how the private `skills.md` SaaS should reuse
`hasna/skills` while keeping the open package generic, local-first, and useful
without private Hasna Tools infrastructure.

## Executive Decision

Keep `hasna/skills` as the upstream engine and source corpus. Build the SaaS as
a wrapper around stable upstream contracts instead of copying the skill engine
into a private-only product.

The SaaS may depend on upstream package APIs, consume its `skills/` corpus, and
propose generic improvements back to `hasna/skills`. It must not push private
tenant state, Stripe billing, AWS deployment, hosted execution secrets, or
skills.md-specific product flows into upstream.

## Source Inventory

| Area | Path | Reuse Decision | SaaS Role |
| --- | --- | --- | --- |
...
| Scheduler | `src/lib/scheduler.ts` | Reference only for V1 SaaS | Good local scheduler semantics, but SaaS scheduling needs PostgreSQL and workers. |
...
| Local server | `src/server/serve.ts` | Demo/reference only | Not a production SaaS backend. |
| Vite dashboard | `dashboard/` | Demo/reference only | Useful UX reference, not the future SaaS web app. |
...
| Workflows | `.github/workflows/*` | Reference only | Upstream CI/publish workflows are not the SaaS AWS release pipeline. |
...
- Validation: keep `parseSkillFrontmatter`, `validateSkillDirectory`, and
  ...
  SaaS ingestion, and local authoring.
...
- Upstream sync automation: keep scripts and docs that make upstream pull/push
  boundaries explicit.
...
- PostgreSQL tenant schema: tenants, users, teams, memberships, API keys,
  skill pins, execution logs, exports, audit logs, credits, and billing
  ledger.
...
- Billing service: Stripe Checkout, Stripe Payment Links when useful, customer
  portal, webhooks, credit grants, refunds, payment approval records, and test
  mode fixtures.
- Web app: future Next.js interface on the same API contracts as CLI/MCP.
- Agent API: versioned REST endpoints and MCP-facing endpoints under the
  skills.md domain.
- AWS deployment: Terraform, ECS, RDS, Redis, S3, ECR, Route 53, ACM, PR
  previews, production tag deploys, smoke tests, rollback, and cleanup.
...
The local scheduler is file/config oriented. SaaS scheduling needs PostgreSQL
state, worker queues, tenant-level authorization, idempotency keys, and
...
2. Sync bundled `skills/*` metadata into the SaaS registry through a
   validation pipeline.
3. Store tenant pins in PostgreSQL while never writing local agent skill
   manifests.
...
Every SaaS wrapper change will need service-level tests once the private app
exists:
...
Stripe sandbox work must verify Checkout, Payment Links if selected, webhooks,
credit grants, failed payment behavior, and idempotent replay handling without
printing secrets.

## Non-Goals

- Do not rename `hasna/skills` or replace it with a private-only fork.
- Do not add `skills.md`, Stripe products, AWS account IDs, or private
  deployment assumptions to upstream-compatible modules.
