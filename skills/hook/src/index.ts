...
/**
 * hook - Claude Code hook scaffold generator
 *
 * Creates a complete hook template in the current directory.
 * Running `hook` without arguments creates the scaffold interactively.
 */
...
function printHelp() {
  console.log(`hook - Claude Code hook scaffold generator

USAGE:
  hook [name]           Create a hook scaffold (interactive if no name)
  hook [options]
...
EXAMPLES:
  hook                  Interactive hook creation
  hook my-hook          Create hook-my-hook scaffold
  hook --event Stop     Create with Stop event
  hook events           List hook events
...
async function main() {
  ...
  if (args[0] === 'validate') {
    ...
    if (!path) {
      ...
      console.error('Usage: hook validate <path>');
      process.exit(1);
    }
