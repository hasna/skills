...
async function makeRequest<T>(
  ...
  if (!apiUrl) {
    throw new ApiError('API URL not configured. Run: manageskill config --set-api-url <url>');
  }

  if (!apiKey) {
    throw new ApiError('API key not configured. Run: manageskill config --set-api-key <key>');
  }
