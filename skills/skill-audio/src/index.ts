...
const SKILL_META = {
  name: 'skill-audio',
  description: 'Audio skill - calls remote API',
  version: '1.0.0',
  commands: `skill-audio <command> [options]`,
...
USAGE:
  skill-audio <command> [options]

COMMANDS:
  Call with any command and it will be forwarded to the remote API server

EXAMPLES:
  skill-audio generate --provider <provider> [options]`);
    return;
  }
  ...
  const isFileSkill = fileSkills.includes('audio');
