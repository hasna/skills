...
const SKILL_META = {
  ...
  description: 'Transform skill - calls remote API',
  version: '1.0.0',
  commands: `skill-transform <command> [options]`,
...
async function main() {
  ...
  if (command === 'help' || args.help) {
    console.log(`Skill Transform CLI

USAGE:
  skill-transform <command> [options]

COMMANDS:
  Call with any command and it will be forwarded to the remote API server

EXAMPLES:
  skill-transform generate --provider <provider> [options]`);
    return;
  }
  ...
  const isFileSkill = fileSkills.includes('transform');
