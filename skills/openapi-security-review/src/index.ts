...
const SKILL = "openapi-security-review";
const TOPIC = "openapi-security-review";
...
function scanProject(root: string): Finding[] {
  ...
  function walk(dir: string, depth = 0) {
    ...
    for (const ent of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (ent.isDirectory() && !ent.name.startsWith(".")) walk(p, depth + 1);
      else if (/openapi|swagger/i.test(ent.name) && /\.(json|ya?ml)$/i.test(ent.name)) {
        const text = readFileSync(p, "utf8");
        if (!/securitySchemes|security:/i.test(text)) {
          findings.push({ severity: "warn", message: "OpenAPI spec may lack security schemes", file: ent.name });
        }
      }
    }
  }
...
function buildReport(payload: ReturnType<typeof formatPayload>, path?: string) {
  ...
  return {
    skill: SKILL,
    topic: TOPIC,
    title: payload.title ?? "Review OpenAPI specs for auth gaps, data exposure, and unsafe endpoint design",
    checklist: CHECKLIST,
    hints: payload.hints ?? [],
    findings,
    summary: {
...
function printHelp() {
  console.log(`${SKILL} — Review OpenAPI specs for auth gaps, data exposure, and unsafe endpoint design
