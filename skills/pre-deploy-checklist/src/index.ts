...
const SKILL = "pre-deploy-checklist";
const TOPIC = "pre-deploy-checklist";
...
function scanProject(root: string): Finding[] {
  ...
  for (const f of ["package.json", ".env.example", "README.md"]) {
    if (!existsSync(join(root, f))) findings.push({ severity: "warn", message: `Missing ${f} before deploy` });
  }
  const pkg = join(root, "package.json");
  if (existsSync(pkg)) {
    const j = JSON.parse(readFileSync(pkg, "utf8"));
    if (!j.scripts?.test) findings.push({ severity: "warn", message: "No test script in package.json" });
    if (!j.scripts?.build) findings.push({ severity: "warn", message: "No build script in package.json" });
  }
...
function buildReport(payload: ReturnType<typeof formatPayload>, path?: string) {
  ...
  return {
    skill: SKILL,
    topic: TOPIC,
    title: payload.title ?? "Pre-deployment checklist for tests, security, env vars, migrations, and rollback readiness",
    checklist: CHECKLIST,
    hints: payload.hints ?? [],
    findings,
    summary: {
...
function printHelp() {
  console.log(`${SKILL} — Pre-deployment checklist for tests, security, env vars, migrations, and rollback readiness
