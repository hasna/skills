...
function scanProject(root: string): Finding[] {
  ...
  for (const ent of readdirSync(src, { withFileTypes: true })) {
    if (ent.isFile() && /\.(ts|tsx|js)$/.test(ent.name)) files++;
  }
  findings.push({ severity: "info", message: `TDD: start with failing test for smallest unit among ~${files || 1} modules` });
...
function buildReport(payload: ReturnType<typeof formatPayload>, path?: string) {
  ...
  return {
    skill: SKILL,
    topic: TOPIC,
    title: payload.title ?? "Test-driven development workflow with red-green-refactor steps for new features",
    checklist: CHECKLIST,
    hints: payload.hints ?? [],
    findings,
    summary: {
...
function printHelp() {
  console.log(`${SKILL} — Test-driven development workflow with red-green-refactor steps for new features
