...
const SKILL = "supply-chain-audit";
const TOPIC = "supply-chain-audit";
...
function scanProject(root: string): Finding[] {
  ...
  if (existsSync(pkg)) {
    ...
    for (const [k, v] of Object.entries(j.scripts ?? {})) {
      if (/preinstall|postinstall|prepare/i.test(k)) {
        findings.push({ severity: "warn", message: `Lifecycle script: ${k}`, file: "package.json" });
      }
      if (/curl|wget|powershell.*irm/i.test(String(v))) {
        findings.push({ severity: "fail", message: `Remote fetch in script ${k}`, file: "package.json" });
      }
    }
  }
...
function buildReport(payload: ReturnType<typeof formatPayload>, path?: string) {
  ...
  return {
    skill: SKILL,
    topic: TOPIC,
    title: payload.title ?? "Offline repository security audit for leaked secrets, risky install scripts, and dependency issues",
    checklist: CHECKLIST,
    hints: payload.hints ?? [],
    findings,
    summary: {
...
function printHelp() {
  console.log(`${SKILL} — Offline repository security audit for leaked secrets, risky install scripts, and dependency issues
