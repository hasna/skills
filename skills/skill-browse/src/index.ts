...
const SKILL_META = {
  name: 'skill-browse',
  description: 'Browse skill - calls remote API',
  version: '1.0.0',
  commands: `skill-browse <command> [options]`,
...
async function main() {
  ...
  if (command === 'help' || args.help) {
    console.log(`Skill Browse CLI

USAGE:
  skill-browse <command> [options]

COMMANDS:
  Call with any command and it will be forwarded to the remote API server

EXAMPLES:
  skill-browse generate --provider <provider> [options]`);
    return;
  }
  ...
  const isFileSkill = fileSkills.includes('browse');
