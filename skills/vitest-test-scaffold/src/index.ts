...
const SKILL = "vitest-test-scaffold";
const TOPIC = "vitest-test-scaffold";
...
function scanProject(root: string): Finding[] {
  ...
  const hasVitest = existsSync(join(root, "vitest.config.ts")) || existsSync(join(root, "vitest.config.js"));
  const hasJest = existsSync(join(root, "jest.config.js")) || existsSync(join(root, "jest.config.ts"));
  if (!hasVitest && !hasJest) findings.push({ severity: "info", message: "No vitest/jest config — scaffold recommended" });
  const src = existsSync(join(root, "src")) ? join(root, "src") : root;
  let modules = 0;
  for (const ent of readdirSync(src, { withFileTypes: true })) {
    if (ent.isFile() && /\.(ts|tsx)$/.test(ent.name)) modules++;
  }
  if (modules) findings.push({ severity: "info", message: `Found ~${modules} TypeScript modules to target for first tests` });
...
function buildReport(payload: ReturnType<typeof formatPayload>, path?: string) {
  ...
  return {
    skill: SKILL,
    topic: TOPIC,
    title: payload.title ?? "Scaffold Vitest unit tests, config, and first test files for TypeScript projects",
    checklist: CHECKLIST,
    hints: payload.hints ?? [],
    findings,
    summary: {
...
function printHelp() {
  console.log(`${SKILL} — Scaffold Vitest unit tests, config, and first test files for TypeScript projects
