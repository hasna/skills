#!/usr/bin/env bun
/**
 * siteanalyze — Analyze any website's design system via Playwright + AI Vision
...
async function fetchPageSource(url: string): Promise<string> {
  const response = await fetch(url, {
    headers: {
      "User-Agent":
        "Mozilla/5.0 (compatible; siteanalyze/1.0; +https://github.com/hasna/skills)",
    },
  });
  if (!response.ok) {
...
async function captureScreenshot(url: string): Promise<string | null> {
  ...
  try {
    ...
    const tmpDir = join(tmpdir(), "siteanalyze");
...
function parseArgs(argv: string[]): { command: string; options: CliOptions } {
  ...
  if (firstArg === "analyze") {
    ...
    // Allow `siteanalyze <url>` shorthand
    command = "analyze";
...
USAGE
  siteanalyze <url> [options]
  siteanalyze analyze --url <url> [options]
  siteanalyze help
...
EXAMPLES
  siteanalyze https://vercel.com
  siteanalyze https://stripe.com --format profile
  siteanalyze https://tailwindcss.com --provider openai
  siteanalyze https://shadcn.com --quick
  siteanalyze analyze --url https://example.com --output ./profile.json
...
async function main(): Promise<void> {
  ...
  if (command === "analyze") {
    if (!options.url) {
      ...
      console.error(
        "Run `siteanalyze help` for usage."
      );
      process.exit(1);
    }
