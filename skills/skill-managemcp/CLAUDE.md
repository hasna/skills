...
Claude-specific instructions for working with the managemcp codebase.

## Overview

`skill-managemcp` is a TypeScript service for managing MCP (Model Context Protocol) servers. It provides a CLI tool, REST API server, and PostgreSQL database for listing, installing, removing, and configuring MCP server connections.
...
- **Config** - CLI config stored in `~/.managemcp.json`, server config via `.env`
