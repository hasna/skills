...
const SKILL = "secrets-scanner-4";
const TOPIC = "secrets-scanner";
const BUNDLE = join(dirname(import.meta.path), "..", "data", "reference.json");

const CHECKLIST = [
  "Scans project files for AWS keys, GitHub tokens, private keys, and JWTs",
  "Uses pattern matching — no network or external secret databases",
  "Reports file path and line context for each finding",
  "Suitable for pre-commit and CI secret gates",
];
    ...
    for (const ent of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (ent.isDirectory()) walk(p, depth + 1);
        ...
        for (const [rx, label] of patterns) {
          if (rx.test(text)) findings.push({ severity: "fail", message: `${label} pattern in ${ent.name}`, file: ent.name });
        }
      }
    }
  }
...
function buildReport(payload: ReturnType<typeof formatPayload>, path?: string) {
  ...
  return {
    skill: SKILL,
    topic: TOPIC,
    title: payload.title ?? "Scan project files for leaked API keys, tokens, passwords, and private keys",
    checklist: CHECKLIST,
    hints: payload.hints ?? [],
    findings,
    summary: {
...
function printHelp() {
  console.log(`${SKILL} — Scan project files for leaked API keys, tokens, passwords, and private keys
