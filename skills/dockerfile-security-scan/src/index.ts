...
const SKILL = "dockerfile-security-scan";
const TOPIC = "dockerfile-security-scan";
...
function scanProject(root: string): Finding[] {
  ...
  for (const ent of readdirSync(root, { withFileTypes: true })) {
    if (!/^dockerfile/i.test(ent.name) && ent.name !== "Dockerfile") continue;
    const text = readFileSync(join(root, ent.name), "utf8");
    if (!/^USER\s+/im.test(text)) findings.push({ severity: "warn", message: "Dockerfile runs as root (no USER)", file: ent.name });
    if (/:latest\b/i.test(text)) findings.push({ severity: "warn", message: "Uses :latest image tag", file: ent.name });
    if (/\b(curl|wget)\b/i.test(text)) findings.push({ severity: "info", message: "curl/wget in RUN — review supply chain", file: ent.name });
  }
...
function buildReport(payload: ReturnType<typeof formatPayload>, path?: string) {
  ...
  return {
    skill: SKILL,
    topic: TOPIC,
    title: payload.title ?? "Scan Dockerfiles for root user, secrets in layers, and container hardening issues",
    checklist: CHECKLIST,
    hints: payload.hints ?? [],
    findings,
    summary: {
...
function printHelp() {
  console.log(`${SKILL} — Scan Dockerfiles for root user, secrets in layers, and container hardening issues
