...
const SKILL_META = {
  name: 'skill-video',
  description: 'Video skill - calls remote API',
  version: '1.0.0',
  commands: `skill-video <command> [options]`,
...
USAGE:
  skill-video <command> [options]

COMMANDS:
  Call with any command and it will be forwarded to the remote API server

EXAMPLES:
  skill-video generate --provider <provider> [options]`);
    return;
  }
  ...
  const isFileSkill = fileSkills.includes('video');
