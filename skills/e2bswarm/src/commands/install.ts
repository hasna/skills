...
e2bswarm spawn --repo <github-url> --tasks <task-list-id> --instances <count>

# Spawn from a local directory
e2bswarm spawn --local ./my-project --tasks <task-list-id> --instances 3
...
- Bun runtime with \`e2bswarm\` installed globally

## Install CLI

\`\`\`bash
bun add -g @hasnaxyz/e2bswarm
\`\`\`
`;
}
