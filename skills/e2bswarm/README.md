...
```bash
# Clone and install
cd ~/Workspace/dev/hasnaxyz/skill/skilldev/e2bswarm
bun install

# Link to Claude Code skills
bun run link
# or manually:
ln -sf $(pwd) ~/.claude/skills/e2bswarm
```
...
```
/e2bswarm spawn --repo git@github.com:org/repo.git --tasks my-task-list --instances 4
```
...
Instance state is persisted to `~/.claude/e2bswarm-state.json` between commands.
