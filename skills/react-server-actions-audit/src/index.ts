...
const SKILL = "react-server-actions-audit";
const TOPIC = "react-server-actions-audit";
...
function scanProject(root: string): Finding[] {
  ...
  function walk(dir: string, depth = 0) {
    ...
    for (const ent of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (/\.(ts|tsx)$/i.test(ent.name)) {
        const text = readFileSync(p, "utf8");
        if (/"use server"/.test(text) && !/auth|session|zod|safeParse/i.test(text)) {
          findings.push({ severity: "warn", message: "Server action without obvious auth/validation", file: ent.name });
        }
      }
    }
  }
...
function buildReport(payload: ReturnType<typeof formatPayload>, path?: string) {
  ...
  return {
    skill: SKILL,
    topic: TOPIC,
    title: payload.title ?? "Audit React Server Actions and RSC data flows for auth leaks and unsafe mutations",
    checklist: CHECKLIST,
    hints: payload.hints ?? [],
    findings,
    summary: {
...
function printHelp() {
  console.log(`${SKILL} — Audit React Server Actions and RSC data flows for auth leaks and unsafe mutations
