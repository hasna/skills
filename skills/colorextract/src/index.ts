#!/usr/bin/env bun
/**
 * colorextract — Extract color palettes from screenshots via AI Vision
...
function printHelp(): void {
  ...
  console.log(`
colorextract — Extract color palettes from screenshots and images via AI Vision

USAGE
  colorextract extract --image <path-or-url> [options]
  colorextract help
...
EXAMPLES
  ...
  colorextract extract --image ./screenshot.png --format profile
  colorextract extract --image ./screenshot.png --output ./colors.json
  colorextract extract --image ./screenshot.png --provider openai
  colorextract extract --image ./screenshot.png --provider gemini --model gemini-2.0-flash
...
async function main(): Promise<void> {
  ...
  if (command === "extract") {
    ...
    if (!options.image) {
      ...
      console.error("Run `colorextract help` for usage.");
      process.exit(1);
    }
