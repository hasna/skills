...
Claude-specific instructions for working with the managehook codebase.

## Overview

`skill-managehook` is a TypeScript service for managing Claude Code hooks. It provides a CLI tool, REST API server, and PostgreSQL database for listing, installing, removing, and configuring hooks.
...
- **Config** - CLI config stored in `~/.managehook.json`, server config via `.env`
